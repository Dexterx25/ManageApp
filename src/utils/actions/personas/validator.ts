export async function Validator (data:any) {
 ...
 if(date_int){
    ...
    const validatorFormat = (date:any) =>{
        ...
        if(format.test(date))return 'Formato de fecha no es valida';
    }
   await validatorFormat(date_int)
    if(dataInt > today)return 'La fecha de ingreso no debe ser mayor que la del registro';
    if(today.getMonth() - dataInt.getMonth() > 1 ) return 'La fecha de ingreso no puede ser menor más de un mes a la de registro';
 }else{
     return 'La fecha de ingreso es obligatoria ingresarla'
 }
 if(area){
     let areas :string[] = ["administracion", "financiera", "compras", "infraestructura", "operacion", "talento humano", "servicios varios"]
        if(areas.findIndex((e:string) => e == area.toLowerCase().trim()) == -1){
 ...
 if(primer_nombre.length || primer_apellido.length || segundo_apellido.length ){
   if(full_name){
        if(primer_nombre.length > 20) return 'El primer nombre no debe ser mayor a 20 caracteres';
        if(segundo_nombre.length > 20) return 'El segundo nombre no debe ser mayor a 20 caracteres';
        if(primer_apellido.length > 20) return 'El primer apellido no debe ser mayor a 20 caracteres';
        if(segundo_apellido.length > 20) return 'El segundo apellido no debe ser mayor a 20 caracteres';
 ...
 if(numero_identificacion){
     if(numero_identificacion.length > 20) return 'El numero de identificación como maximo debe tener 20 caracteres';
        function verificationNumber(val:any){
            const verifi2 = /^[0-9|A-Z]+$/i
            if(verifi2.test(val) == false)return 'El numero de identificacion debe estar entre A y Z mayusculas o minusculas y digitos del 0 al 9 y sin acentos ni "ñ"'
        }
 ...
 if(otros_nombres){
  if(otros_nombres.length > 50)return 'Otros nombres debe ser maximo 50 caracteres';
  function regexValidate(val:any) {
      const validation1 = /^[A-Z]+$/
      const validation2 = /[áéíóúñÑÄËÏÖÜÂÊÎÔÛáàèìòù]/
      if(validation1.test(val) == false || validation2.test(val) == true) {
        if(validation2.test(val) == true) return 'No se permiten ni acentos ni "Ñ" para "Otros nombres"'
        if(validation1.test(val) == false) return 'Los valores de "Otros nombres" deben ser de A a la Z en mayusculas';
      } 
  }
  for (let i = 0; i < otros_nombres.length; i++) {
      const character = otros_nombres.charAt(i);
     let responValidation :any =  await regexValidate(character)
       if(responValidation){
 ...
 if(tipo_identificacion){
     console.log('TIPO IDENTIFICACION---->', tipo_identificacion)
     let conditions : string[] = ['C.C', 'C.E', 'PA', 'PE'] 
